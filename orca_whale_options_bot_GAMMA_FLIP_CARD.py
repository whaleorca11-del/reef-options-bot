...
def render_trade_card(s):
    ...
    # Gamma Flip / market regime panel
    d.rounded_rectangle((80, 945, 1000, 1085), radius=22, fill=panel2, outline=line, width=2)
    d.text((105, 965), "GAMMA FLIP", font=_font(22, True), fill=blue)
    d.text((105, 1005), f"${gamma_flip:.2f}" if gamma_flip > 0 else "N/A", font=_font(31, True), fill=white)

    d.text((325, 965), "STOCK", font=_font(22, True), fill=blue)
    d.text((325, 1005), f"${gamma_stock:.2f}" if gamma_stock > 0 else "N/A", font=_font(31, True), fill=white)

    d.text((525, 965), "DISTANCE", font=_font(22, True), fill=blue)
    dist_color = green if gamma_status in ("BULLISH SIDE", "BEARISH SIDE") else red if gamma_status == "EXIT WARNING" else muted
    dist_text = f"{gamma_distance:+.2f} ({gamma_distance_pct:+.2f}%)" if gamma_flip > 0 else "N/A"
    d.text((525, 1005), dist_text, font=_font(25, True), fill=dist_color)

    d.text((795, 965), "STATUS", font=_font(22, True), fill=blue)
    d.text((795, 1005), gamma_status, font=_font(21, True), fill=dist_color)
    d.text((105, 1050), f"{gamma_side} FLIP  •  NET GEX {gamma_regime}", font=_font(19, True), fill=muted)

    # Progress
    d.line((120, 1150, 930, 1150), fill=(93, 111, 126), width=5)
    points = [120, 322, 525, 727, 930]
    for i, x in enumerate(points):
        hit = i < achieved
        fill = green if hit else panel
        outline = green if hit else (130, 151, 167)
        d.ellipse((x-25, 1125, x+25, 1175), fill=fill, outline=outline, width=4)
        if hit:
            d.text((x-10, 1129), "✓", font=_font(26, True), fill=white)
        d.text((x-20, 1188), f"T{i+1}", font=_font(20, True), fill=white)
    d.text((95, 1090), f"TARGET PROGRESS   {achieved}/5", font=_font(24, True), fill=white)

    # Contract details table
    d.rounded_rectangle((80, 1250, 1000, 1490), radius=24, fill=panel2)
    details = [
        ("TICKER", s["symbol"]),
    ...
    # Footer / time / score
    d.text((95, 1520), f"Entry {format_entry_time_et(s.get('created_at'))}", font=_font(20), fill=muted)
    d.text((430, 1520), f"Score {s['score']}/100", font=_font(20), fill=muted)
    d.text((695, 1520), "Powered by ORCA WHALE OPTIONS BOT", font=_font(20, True), fill=blue)
