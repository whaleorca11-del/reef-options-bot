...
AUTO_MONITOR_SECONDS = int(os.getenv("AUTO_MONITOR_SECONDS", "20"))
...
def extract_snapshot_price(snapshot):
    """Best available current option price from a Massive contract snapshot."""
    ...
    bid = safe_float(quote.get("bid") or quote.get("bid_price"))
    ask = safe_float(quote.get("ask") or quote.get("ask_price"))
    last = safe_float(trade.get("price"))
    day_close = safe_float(day.get("close"))

    if last > 0:
        return last, "Massive Last"
    if bid > 0 and ask > 0 and ask >= bid:
        return (bid + ask) / 2, "Massive Mid"
    if ask > 0:
        return ask, "Massive Ask"
    if bid > 0:
        return bid, "Massive Bid"
    if day_close > 0:
        return day_close, "Massive Day Close"
    return 0.0, "Unavailable"
...
def _load_trade_store():
    ...
    Automatically migrates the old V6 single-trade state when present.
    """
    raw = load_state()

    if isinstance(raw, dict) and isinstance(raw.get("trades"), dict):
        return raw

    if isinstance(raw, dict) and raw.get("symbol"):
        symbol = str(raw.get("symbol", "")).upper().strip()
        if symbol:
            return {"trades": {symbol: raw}}
...
def get_active_trade(symbol):
    ...
    trade = store.get("trades", {}).get(symbol)
...
def _update_option_price_locked(option_ticker, option_price):
    """
    Update ONLY the locked option contract price.

    V9 behavior:
    - Detect every newly crossed target from the OPTION CONTRACT price.
    - Edit the SAME Telegram card immediately when TP1/TP2/TP3/TP4/TP5 is crossed.
    - If one Massive snapshot jumps across several targets, publish the target
      milestones sequentially to the same card instead of silently skipping them.
    - Stop loss is also published immediately.
    """
    store = _load_trade_store()
    trades = store.get("trades", {})
    ...
    # STOP LOSS: based ONLY on locked option price.
    if price <= safe_float(state["stop_loss"]):
        ...
        telegram_edit_card_retry(message_id, state)

        print(
            f"🛑 STOP LOSS | {incoming} = {price:.2f} | "
            f"{matched_symbol}"
        )
    ...
    # Determine how many targets the locked OPTION CONTRACT has reached.
    new_achieved = previous_achieved
    highest = safe_float(state["highest_price"])

    while (
        new_achieved < 5
        and highest >= safe_float(state["targets"][new_achieved])
    ):
        new_achieved += 1

    # Publish every newly reached target to the SAME Telegram card.
    if new_achieved > previous_achieved:
        for milestone in range(previous_achieved + 1, new_achieved + 1):
            state["achieved"] = milestone
            ...
            print(
                f"🎯 TARGET {milestone} HIT | {incoming} | "
                f"option {price:.2f} | {matched_symbol}"
            )

            # Tiny separation so Telegram receives distinct sequential edits
            # when one price snapshot crosses more than one target.
            if milestone < new_achieved:
                time.sleep(0.35)

    else:
        # No target crossed: keep state current without unnecessary Telegram edit.
        state["achieved"] = previous_achieved
        trades[matched_symbol] = state
        _save_trade_store(store)
...
def main():
    ...
    print("=" * 60)
    print("REEF ONE FILE BOT V9 - HARD LOCK + TARGET UPDATES")
    print("=" * 60)
