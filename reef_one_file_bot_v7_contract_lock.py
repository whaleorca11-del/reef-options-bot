...
AUTO_MONITOR_SECONDS = int(os.getenv("AUTO_MONITOR_SECONDS", "60"))
...
def create_trade(symbol, direction, stock_price):
    """
    Create ONE locked option trade per stock.

    If the same stock already has an ACTIVE trade, ALL new TradingView
    signals for that stock are ignored until the option trade is stopped
    or completed. The existing contract is never re-selected or chased.
    """
    symbol = str(symbol or "").upper().strip()
    ...
    if isinstance(existing, dict) and existing.get("status") == "active":
        print(
            f"🔒 CONTRACT LOCK: ignored duplicate {symbol} {direction} | "
            f"keeping {existing.get('contract_ticker')}"
        )
        result = dict(existing)
        result["_ignored_duplicate"] = True
        return result

    # TradingView supplies direction only. Massive selects the contract ONCE.
    ...
    # Send exactly one Telegram card when the contract is first locked.
    ...
    print(
        f"🔒 CONTRACT LOCKED: {symbol} {direction} | "
...
def update_option_price(option_ticker, option_price):
    ...
    The underlying stock price is never used here.
    """
    store = _load_trade_store()
    trades = store.get("trades", {})
    ...
    # Stop-loss is based ONLY on the LOCKED OPTION CONTRACT price.
    ...
    # Edit the SAME Telegram card; never send a second trade card.
    if state.get("message_id"):
        telegram_edit_card(state["message_id"], state)

    print(
        f"📍 OPTION {incoming} = {price:.2f} | "
        f"{matched_symbol} targets {state['achieved']}/5 | "
        ...
        try:
            if path == "/signal":
                ...
                self.json_response(202, {
                    ...
                    "contract_locked": state["contract_ticker"],
