...
def monitor_active_trade_once():
    """Automatically update the currently selected contract."""
    state = load_state()
    ...
    if not symbol or not contract:
        return {"accepted": False, "error": "active trade missing symbol/contract"}

    snapshot = massive_contract_snapshot(symbol, contract)
    price, source = extract_snapshot_price(snapshot)
    if price <= 0:
        return {"accepted": True, "ignored": "no usable option price"}
