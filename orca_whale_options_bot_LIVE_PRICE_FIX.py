...
AUTO_SCAN_MIN_SCORE = float(os.getenv("AUTO_SCAN_MIN_SCORE", "72"))
AUTO_SCAN_MIN_EDGE = float(os.getenv("AUTO_SCAN_MIN_EDGE", "2"))
AUTO_SCAN_MIN_VOLUME = float(os.getenv("AUTO_SCAN_MIN_VOLUME", "30"))
...
def _massive_ts_seconds(value):
    """Normalize Massive timestamps (ns/us/ms/s) to Unix seconds."""
    try:
        v = float(value or 0)
    except Exception:
        return 0.0
...
def extract_snapshot_price(snapshot):
    """
    Fresh option pricing with timestamp awareness.

    Uses the newest usable market event instead of blindly trusting a stale
    quote midpoint. Quote midpoint is preferred when quote data is fresh.
    If a newer trade exists, its price can update the card instead of leaving
    the contract frozen on an older quote.
    """
    ...
    now_ts = time.time()
    quote_age = (now_ts - quote_ts) if quote_ts > 0 else None
    trade_age = (now_ts - trade_ts) if trade_ts > 0 else None

    quote_price = 0.0
    quote_source = "none"

    if midpoint > 0:
        quote_price = midpoint
        quote_source = "quote_midpoint"
    elif bid > 0 and ask > 0 and ask >= bid:
        quote_price = (bid + ask) / 2.0
        quote_source = "quote_midpoint_calc"
    elif bid > 0:
        quote_price = bid
        quote_source = "quote_bid"
    elif ask > 0:
        quote_price = ask
        quote_source = "quote_ask"

    # A quote with no timestamp is still usable, but when Massive gives both
    # timestamps, never let an older quote freeze a newer trade.
    if quote_price > 0 and last_trade > 0:
        if trade_ts > 0 and quote_ts > 0 and trade_ts > quote_ts:
            return last_trade, f"last_trade_newer(age={trade_age:.1f}s)"
        if quote_age is not None and quote_age > 120 and trade_age is not None and trade_age < quote_age:
            return last_trade, f"last_trade_fresher(age={trade_age:.1f}s)"
        age_txt = "na" if quote_age is None else f"{quote_age:.1f}s"
        return quote_price, f"{quote_source}(age={age_txt})"
    ...
    if last_trade > 0:
        age_txt = "na" if trade_age is None else f"{trade_age:.1f}s"
        return last_trade, f"last_trade(age={age_txt})"
...
def scan_symbol_options(symbol):
    ...
    if (
        ...
        direction = "CALL"
        gamma_override = safe_float(cb.get("score")) >= 85 and call_strength >= put_strength + 5
        if not gamma_flip_allows(direction, stock_price, gamma_info) and not gamma_override:
            direction = None
    elif (
        ...
        direction = "PUT"
        gamma_override = safe_float(pb.get("score")) >= 85 and put_strength >= call_strength + 5
        if not gamma_flip_allows(direction, stock_price, gamma_info) and not gamma_override:
            direction = None
